...
export default function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      // Parse technical proposal if string
      let techProposal = project.technicalProposal;
      if (typeof techProposal === 'string') {
        try { techProposal = JSON.parse(techProposal); } catch { techProposal = null; }
      }
      
      const offerData = {
        projectId: project.id,
        projectName: project.name,
        clientName: project.requirements?.clientName || project.client || 'Client',
        clientCompany: project.requirements?.projectName || '',
        clientEmail: clientEmail || '',
        description: project.requirements?.description || project.description || '',
        requirements: project.requirements,
        technicalProposal: techProposal as Record<string, unknown> || undefined,
      };
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `Oferta-${project.id}.pdf`;
  ...
  const handleSendOffer = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert('Eroare: ' + (data.error || 'Nu am putut trimite oferta'));
      }
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              <Link href="/admin/ai-studio" className="text-slate-400 hover:text-white transition-colors">
                ← Înapoi
              </Link>
              <Image src="/logo.png" alt="PREV-COR TPM" width={40} height={40} className="rounded-lg" />
              <div>
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6">
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  disabled={generatingPdf || !project.technicalProposal}
                  className="w-full py-2 px-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 transition-colors font-medium"
                >
                  {generatingPdf ? '⏳ Se generează...' : '📄 Descarcă Ofertă PDF'}
                </button>
                
                {/* Send Offer by Email */}
                <button
                  onClick={() => setShowOfferModal(true)}
                  disabled={!project.technicalProposal}
                  className="w-full py-2 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 transition-colors font-medium"
                >
                  📧 Trimite Ofertă prin Email
                </button>
                
                {!project.technicalProposal && (
                  <p className="text-xs text-amber-400 text-center">
                    ⚠️ Generează mai întâi Soluția Tehnică (Faza 2)
                  </p>
                )}
                ...
                <Link
                  href={`/admin/ai-studio/project/${id}/edit`}
                  ...
                  onClick={() => {
                    if (confirm('Sigur vrei să ștergi acest proiect?')) {
                      fetch(`/api/ai-studio/projects/${id}`, { method: 'DELETE' })
                        .then(() => router.push('/admin/ai-studio'));
                    }
                  }}
      ...
      {showOfferModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-2xl p-6 w-full max-w-md border border-slate-700">
            <h3 className="text-xl font-bold text-white mb-4">📧 Trimite Ofertă</h3>
            
            <div className="space-y-4">
              <div>
                <label className="block text-slate-300 text-sm mb-1">Proiect</label>
                <p className="text-white font-medium">{project.name}</p>
              </div>
              
              <div>
                <label className="block text-slate-300 text-sm mb-1">Client</label>
                <p className="text-white">{project.requirements?.clientName || project.client || '-'}</p>
              </div>
              
              <div>
                <label className="block text-slate-300 text-sm mb-1">Email destinatar *</label>
                <input
                  type="email"
                  value={clientEmail}
              ...
              <div className="flex gap-3 pt-4">
                <button
                  onClick={() => { setShowOfferModal(false); setClientEmail(''); }}
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 font-medium"
                >
